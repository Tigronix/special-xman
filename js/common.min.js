...
const from4k = window.matchMedia("(min-width: 2000px)");
    ...
    if (!isIe11 && !isEdge) {
        if (fakeBlock) {
            ...
            const scrollToContent= function() {
                ...
                const contentOffset = $('.scroll-section__wrap').offset().top;
...
SX.historySlider = function() {
    ...
    const removeHeader = function(){
        const $pageHeaderTop = $('.page-header--top');
        const $pageHeaderBot = $('.page-header--bot');
        const $pageheaderTopMenu = $pageHeaderTop.find('.history__menu');

        if(fromDesktop.matches){
            $pageheaderTopMenu.remove();
        }else if(atHorizontalTablet.matches){
            $pageHeaderBot.remove();
        }else if(atVerticalTablet.matches || atMobile.matches){
            $pageHeaderBot.remove();
        }
    };
    ...
    const historyTypeSliderAppendArrows = function(){
        $(historyTypeSlider).each(function(index) {
            const arrows = '<svg class="hover mouseDown icon icon-arrow history__type-slider-arrow history__type-slider-arrow--prev"><use xlink:href="#icon-arrow"></use></svg> <svg class="hover mouseDown icon icon-arrow history__type-slider-arrow history__type-slider-arrow--next"><use xlink:href="#icon-arrow"></use></svg>';
            $('.history-type-slider-' + index + ' .history__type-slider-item-wrap').append(arrows);
        });
    };
    ...
    const historyTypeSliderInit = function() {
     ...
     SX.historyTypeSliderArrowDisabled = function() {
        $('.history__type-slider').each(function(){
            $(this).find('.history__type-slider-arrow').removeClass('hide');

            $(this).find('.slick-slide').first().find('.history__type-slider-arrow--prev').addClass('hide');
            $(this).find('.slick-slide').last().find('.history__type-slider-arrow--next').addClass('hide');
        });
    };
    ...
    const historyTabs = function() {
        ...
        $btn.each(function() {
            $(this).on('click', function() {
                ...
                const activeBtnsLength = $(this).siblings('.activeHistory').length;
    ...
    const historyNavPosition = function(){
        const getPosition = function(){
            if(fromHorizontalTablet.matches){
                const $historyNav = $('.history__nav');
                const $navItems = $('.history__type-slider-item');

                $navItems.each(function(){
                    let navPosition = $('.history__type-slider-item').find('p').offset().left;

                    if(atHorizontalTablet.matches){
                        navPosition = navPosition + 70;
                    }

                    $historyNav.css('left', navPosition);
                });
            }
        };
    ...
    historySliderAnimationFix = function(){
        $('.history__type-slider-slide-wrapper:first-child').each(function(){
            $(this).find('.history__type-slider-item-wrap').removeClass('animated-1s fadeInUpBig').addClass('animated-1s fadeInUpBig');
        });
    };
    ...
    historyTypeSliderAppendArrows();
    SX.historyTypeSliderArrowDisabled();
    historyTabs();
    nav();
    historyNavPosition();
    historySliderAnimationFix();
}
...
SX.scrollToMainStatic = function() {
    ...
    if (fromDesktop.matches) {
        ...
        const sceneInMobile = new ScrollMagic.Scene({
                triggerElement: '.history',
                offset: 0,
            })
            .setClassToggle('.legacy', 'scrolled')
            .addTo(controllerStatic);
    }
};
...
SX.fancybox = function() {
    $('.js-fancybox').fancybox({
        ...
        afterLoad: function(instance, current) {
            ...
            if($('.global-wrapper').hasClass('global-wrapper--history')){
                $('.fancybox-button--arrow_left').css('display', 'none');
                $('.fancybox-button--arrow_right').css('display', 'none');
            }

            if($('.global-wrapper').hasClass('global-wrapper--people')){
                $('.fancybox-button--arrow_left').css('display', 'none');
                $('.fancybox-button--arrow_right').css('display', 'none');
            }
        ...
        afterShow: function(){
            const $arrPrev = $('.fancybox-button--arrow_left');
            const $arrNext = $('.fancybox-button--arrow_right');
            const isDisabledPrev = $arrPrev.attr('disabled');
            const isDisabledNext = $arrNext.attr('disabled');

            if(isDisabledPrev){
                $arrPrev.addClass('disabled').removeAttr('disabled');
            }else if(isDisabledNext){
                $arrNext.addClass('disabled').removeAttr('disabled');
            }else{
...
SX.peoplePopup = function(){
    const $btns = $('.js-people-popup-btn');
    const $popup = $('.js-people-popup');
    const $btnClose = $('.js-people-popup-close');
    const $globalWrapper = $('.global-wrapper');

    SX.peoplePopupOpen = function(elem){
        const $popup = elem.closest('.people-slider__slide-wrap').find('.js-people-popup');

        $popup.addClass('active');
        $globalWrapper.addClass('peoplePopupActive');
    };

    SX.peoplePopupClose = function(){
        const peopleSliderOffset = $('.people-slider__button').offset().top;

        $popup.removeClass('active');
        $globalWrapper.removeClass('peoplePopupActive');

        setTimeout(function(){
            $('html, body').scrollTop(peopleSliderOffset);
        }, 100);
    };

    $btns.each(function(){
        $(this).off('click.people-popup-open').on('click.people-popup-open', function(){
            SX.peoplePopupOpen($(this));
        });
    });

    $btnClose.off('click.people-popup-close').on('click.people-popup-close', function(){
        SX.peoplePopupClose();
    });

    $(document).off('keydown.people-popup-close-esc').on('keydown.people-popup-close-esc', function onKeyDown(evt) {
        if (evt.keyCode === SX.ESC_CODE) {
            SX.peoplePopupClose();
        }
    });
};
