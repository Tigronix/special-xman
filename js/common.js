...
SX.mainScrollAnimation = function(){
    ...
    fakeItems.each(function(i, it, array){
        ...
        const tweenIn = function(index){
            return new TimelineMax()
            .fromTo($('#'+title+index), 0.1, {left:0, x: titleAway}, {x:'0', left:0, ease: animationDuration})
...
SX.legacyStaticHover = function(){
 ...
 SX.historySlider = function() {
    ...
    const historyTypeSliderSettings = function(){
        return {
          dots: false,
          arrows: false,
          infinite: false,
    ...
    const historyTypeSliderInit = function(){
        return $(historyTypeSlider).slick(historyTypeSliderSettings());
    };
...
SX.imgGallery = function(){
    const galleryBtns = $('[data-lightbox]');
    const galleryClose = $('.lb-close');
    const galleryNav = $('.lb-nav > *');

    galleryNav.addClass('hover mouseDown');
    galleryClose.addClass('hover mouseDown');

    galleryBtns.on('click', function(){
...
SX.filterMenu = function(){
    ...
    if(historyElem && fromDesktop.matches){
        ...
        const tweenIn = function(){
            new TimelineMax()
            .staggerFromTo(menuLinks, 0.5, {y:'0', opacity:'1'}, {y:'-80', opacity:'0', ease: animationDuration}, 0.1)
            .staggerFromTo(filterMenuLinks, 0.5, {y:'-80', opacity:'0'}, {y:'0', opacity:'1', ease: animationDuration}, 0.1)
            ;
        };

        const tweenOut = function(){
            new TimelineMax()
            .staggerFromTo(filterMenuLinks, 0.5, {y:'0', opacity:'1'}, {y:'-80', opacity:'0', ease: animationDuration}, 0.1)
            .staggerFromTo(menuLinks, 0.5, {y:'-80', opacity:'0'}, {y:'0', opacity:'1', ease: animationDuration}, 0.1)
            ;
        };
...
SX.historyCircle = function(){
    ...
    if(circleIs){
        const moveCircle = function(){
            ...
            $circle.css({'left': circlePosition + 'px', 'top': ''});

            if(atVerticalTablet.matches | atMobile.matches){
                circlePosition = imgOffsetTop;

                $circle.css({'top': circlePosition + 'px', 'left': ''});
            }
        };
    ...
    $(window).on('load resize', function(){
        ...
        if(atVerticalTablet.matches){
            const historyMenu = new TimelineMax()
                .staggerFromTo($btns, 0.5, {y:'0', opacity:'1'}, {y:'-30', opacity:'0', display:'none', ease: animationDuration}, 0.1)
                .fromTo('.menu', 0.5, {y: '0', opacity:'1'}, {y:'-30', opacity:'0', display:'none', ease: animationDuration}, -0.5)
                .fromTo('.logo', 0.5, {y: '0', opacity:'1'}, {y:'-30', opacity:'0', ease: animationDuration})
                .fromTo('.history__menu', 0.001, {display:'none'}, {display:'flex'})
                .staggerFromTo($historyBtn, 0.5, {y:'-30', opacity:'0'}, {y:'0', opacity:'1', visibility:'visible', ease: animationDuration}, 0.1)
                .fromTo('.social__close', 0.5, {y: '-30', opacity:'0'}, {y:'0', opacity:'1', display:'flex', ease: animationDuration})
                ;
            historyMenu.stop();
        ...
        if(atVerticalTablet.matches | atMobile.matches){
            const menu = new TimelineMax()
                ...
                .fromTo('.logo', 0.5, {y: '0', opacity:'1'}, {y:'-30', opacity:'0', ease: animationDuration})
                .fromTo('.social__close', 0.5, {y: '-30', opacity:'0'}, {y:'0', opacity:'1', display:'flex', ease: animationDuration})
                .staggerFromTo('.social__link', 0.5, {y:'-30', opacity:'0'}, {visibility: 'visible', y:'0', opacity:'1', ease: animationDuration}, 0.1)
                ;
            menu.stop();
        ...
        if(atMobile.matches){
            const historyMenuMobile = new TimelineMax()
                .fromTo('.history__menu', 0.0001, {visibility: 'hidden'}, {visibility: 'visible'})
                .staggerFromTo($historyBtn, 0.5, {y:'-30', opacity:'0'}, {y:'0', opacity:'1', ease: animationDuration}, 0.1)
                ;
...
SX.adaptiveBg = function(){
    const $elem = $('.js-adaptive-bg');
    const $popupContainer = $('.lb-container');
    console.log($popupContainer);

    $elem.on('click', function(){
        const bgDesktop = $(this).data('bg-desktop');
        const bgHorizontal = $(this).data('bg-horizontal-tablet');
        const bgVertical = $(this).data('bg-vertical-tablet');
        const bgMobile = $(this).data('bg-mobile');

        const changeBg = function(url){
            $popupContainer.css({'background-image':'url("'+ url +'")'});
        };

        const bgUrl = function(){
            if(fromDesktop.matches){
                changeBg(bgDesktop);
            }else if(atHorizontalTablet.matches){
                changeBg(bgHorizontal);
            }else if(atVerticalTablet.matches){
                changeBg(bgVertical);
            }else if(atMobile.matches){
                changeBg(bgMobile);
